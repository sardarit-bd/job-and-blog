...
export default function Header({ auth }) {
    ...
    return (
        <header className="sticky top-0 z-[60] w-full border-b border-slate-200/60 dark:border-slate-800/60 bg-emerald-50/80 dark:bg-slate-950/80 backdrop-blur-xl transition-all">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center h-16 sm:h-20">
                    
                    {/* Logo Section */}
                    <Link href="/" className="flex items-center gap-2 group">
                        <div className="w-10 h-10 bg-[#F8721B] rounded-xl flex items-center justify-center shadow-lg shadow-indigo-500/30 group-hover:rotate-6 transition-transform">
                    ...
                    <div className="flex items-center gap-3">
                        <Link
                            ...
                            className={`px-5 py-2.5 rounded-2xl text-sm font-bold transition-all active:scale-95 shadow-sm
                                ${auth?.user 
                                    ? "bg-[#F8721B] dark:bg-slate-800 text-slate-900 dark:text-white dark:hover:bg-slate-700" 
                                    : "bg-[#F8721B] text-white shadow-indigo-500/20"
                                }`}
                        >
            ...
            >
                <div className="flex flex-col space-y-2 px-4">
                    ...
                    <div className="pt-4 mt-4 border-t border-slate-100 dark:border-slate-800">
                        <Link 
                            href={auth?.user ? "/dashboard" : "/login"} 
                            className="flex justify-center w-full py-4 rounded-2xl bg-[#F8721B] text-white font-black shadow-lg shadow-indigo-500/30"
                        >
