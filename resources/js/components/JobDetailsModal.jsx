      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sm:gap-12">
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <div className="flex flex-wrap gap-2">
            {job.remote_statuses?.map((status, i) => (
              <span key={i} className="px-4 py-1.5 rounded-full text-[11px] font-bold uppercase tracking-widest bg-[#F2EBE6] text-slate-800 border border-slate-200">
                {status}
              </span>
            ))}
          </div>
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-2">
                {job.specialities.map((spec, i) => (
                  <span key={i} className="px-4 py-2 rounded-xl text-sm font-semibold bg-[#F2EBE6] text-slate-800 border border-slate-200">
                    {spec}
                  </span>
                ))}
              </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="p-6 bg-[#F2EBE6] rounded-[2rem] border border-slate-200/50 shadow-xl shadow-slate-200/30 space-y-6">
            <h4 className="font-bold text-slate-900 text-xs uppercase tracking-widest text-center border-b border-slate-200 pb-4">Job Summary</h4>
            ...
            <div className="pt-4 space-y-3">
              {loadingStatus ? (
                ...
                <div className="w-full py-3.5 text-center px-4 bg-red-50 text-red-600 rounded-2xl text-xs font-bold uppercase border border-red-100">
                  Restricted Access
                </div>
              ) : job.already_applied ? (
